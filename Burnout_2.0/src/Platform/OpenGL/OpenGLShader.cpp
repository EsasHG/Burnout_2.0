...
namespace Burnout
{
	...
	static GLenum ShaderTypeFromString(const std::string& type)
	{
		...
		BO_CORE_ASSERT(false, "Unknown Shader Type!");
		return 0;
	...
	std::string OpenGLShader::ReadFile(const std::string& filepath)
	{
		...
		{
			BO_CORE_ERROR("Could Not Open File! '{0}'", filepath);
		}
		return result;
	}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			BO_CORE_ASSERT(eol != std::string::npos, "Syntax error");
			size_t begin = pos + typeTokenLength + 1;
			std::string type = source.substr(begin, eol - begin);
			BO_CORE_ASSERT(ShaderTypeFromString(type), "Invalid Shader Type specified");

			size_t nextLinePos = source.find_first_not_of("\r\n", eol);
			BO_CORE_ASSERT(nextLinePos != std::string::npos, "Syntax error");
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		for (auto& kv : shaderSources)
		{
			...
			if (isCompiled == GL_FALSE)
			{
				...
				BO_CORE_ERROR("{0}", infoLog.data());
				BO_CORE_ASSERT(false, "Shader compilation failure!")
					break;
			}
		...
		if (isLinked == GL_FALSE)
		{
			...
			BO_CORE_ERROR("{0}", infoLog.data());
			BO_CORE_ASSERT(false, "Shader link failure!");
			return;
		}
		for (auto id : glShaderIDs)
		{
	...
	void OpenGLShader::UploadUniformIntArray(const std::string& UniformName, int* values, uint32_t count) const
	{
		GLint location = glGetUniformLocation(m_RendererID, UniformName.c_str());
